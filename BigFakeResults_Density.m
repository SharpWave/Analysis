...
save outs_Density.mat

load outs_Density.mat;
...
for i = 1:length(sizes)
    for j = 1:length(seeds)
        ...
        PicaPartner{i,j} = outs(i,j).PicaPartner; % list, for every fake ROI, of matching PCA/ICA ROI #, 0 if none
        TnspsPartner{i,j} = outs(i,j).TnspsPartner; % list, for every fake ROI, of matching Tenaspis ROI #, 0 if none
    end
end
